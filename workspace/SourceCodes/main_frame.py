...
class MainFrame(Frame, User):
    ...
    # [추가] 환자용: 식단 추천 창 열기
    def openRecommendMenuFrame(self):
        if len(self.user.getDataList()) == 0:
            messagebox.showinfo('알림', '건강/식단 데이터가 입력되지 않았습니다.\n해당 작업을 먼저 수행하세요.')
            return

        self.recommendMenuFrame = RecommendMenuFrame(self, self.user)
        self.recommendMenuFrame.place(x = 0, y = 0)

    # Direct message 실행
    def runDirectMessage(self):
        directMessageWindow = Tk()
        directMessageWindow.title('{}님의 메시지함'.format(self.user.getId()))
        directMessageWindow.geometry('350x700')
        directMessageFrame = DirectMessageFrame(directMessageWindow, self.user)
        directMessageFrame.place(x = 0, y = 0)
        directMessageWindow.mainloop()

    # [추가] 환자용: 뱃지 개수 확인 창 열기
    def openCheckBadgeCountFrame(self):
        self.checkBadgeCountFrame = CheckBadgeCountFrame(self, self.user)
        self.checkBadgeCountFrame.place(x = 0, y = 0)
    ...
    def __init__(self, window: Frame, user: User) -> None:
        ...
        self.directMesssageButton = Button(self, text = '📩', font = ('Arial', 20, 'bold'), bg = '#09FFFA',\
                borderwidth = 0, command = lambda: self.runDirectMessage())

        self.notificationButton.place(x = 10, y = 10)
        self.directMesssageButton.place(x = 60, y = 10)
        ...
        self.F1A4_Button = Button(self, text = '식단 추천', font = ('Arial', 12, 'bold'), bg = 'white',\
                width = 12, activebackground = '#09FFFA', borderwidth = 1, command = lambda: self.openRecommendMenuFrame())
        self.F1A4_Button.place(x = 85, y = 320)
        ...
        self.F2B_Button = Button(self, text = '배지 개수 확인', font = ('Arial', 12, 'bold'), bg = 'white',\
                width = 12, activebackground = '#09FFFA', borderwidth = 1, command = lambda: self.openCheckBadgeCountFrame())
